...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // Store token
  await client.from('password_reset_tokens').insert({ username, token, expires_at: expiresAt })

  // Send email
  const resetUrl = `${process.env.NEXT_PUBLIC_APP_URL || 'https://ai-companion-frontend-iota.vercel.app'}/reset-password?token=${token}`

  const resend = new Resend(process.env.RESEND_API_KEY)
  await resend.emails.send({
    from: 'HighView SEP AI Coach <onboarding@resend.dev>',
    to: email.trim(),
    ...
    html: `
      <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; padding: 32px;">
        ...
        <p style="color: #64748b; margin-bottom: 24px;">
          Hi ${username}, we received a request to reset your HighView SEP AI Coach password.
        ...
        <p style="color: #94a3b8; font-size: 12px;">HighView · SEP AI Coach</p>
      </div>
    `,
  })
